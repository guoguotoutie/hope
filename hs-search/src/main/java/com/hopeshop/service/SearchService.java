...
public class SearchService {
    ...
    public Goods buildGoods(Spu spu) {
        ...
        // 查询详情
        SpuDetail spuDetail = this.goodsClient.querySpuDetById(spu.getId());
    ...
    private List<Map<String, Object>> getSpec(Long cid, QueryBuilder query) {
        try {
            ...
            Specification specification = this.specificationClient.querySpecByCategoryId(cid);

            List<Map<String, Object>> specs = new ArrayList<>();

            NativeSearchQueryBuilder queryBuilder = new NativeSearchQueryBuilder();
            queryBuilder.withQuery(query);
            List<Object> params = JSON.parseArray(specification.getSpecifications());
            // 聚合规格参数
            params.forEach(p -> {
                Map<String,Object> map = JSON.parseObject(p.toString(),HashMap.class);
                Set<String> sets = map.keySet();
